...
LOCK_FILE="${LOCK_FILE:-/tmp/atri-rpa-passo3.lock}"
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/passo3_job_$(date +%Y%m%d).log"
...
with_lock() {
  if command -v flock >/dev/null 2>&1; then
    flock -n 9 || {
      log "Outro job do RPA ainda esta em execucao. Saindo."
      exit 0
    }
    "$@"
  else
...
run_job() {
  ...
  log "Disparando job Passo 3 em ${API_BASE_URL}"
  request POST "/passo3/fechamento?${query}" "$body"
